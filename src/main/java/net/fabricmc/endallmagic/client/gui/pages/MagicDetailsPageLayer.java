...
public class MagicDetailsPageLayer extends PageLayer {
	...
	private void buttonTooltip(ButtonWidget buttonIn, MatrixStack matrices, int mouseX, int mouseY) {
		ScreenButtonWidget button = (ScreenButtonWidget)buttonIn;
		Identifier lvl = new Identifier("endallmagic:level");
		Identifier key = button.key();
		
		if(key.equals(lvl)) {
			 //int requiredXp = EndAllMagic.getConfig().requiredXp(this.client.player);
			int currentXp = this.client.player.experienceLevel;
			 String progress = "(" + currentXp + "/" + /*requiredXp + */  ")";
			 Text tooltip = (Text.translatable("endallmagic.gui.page.attributes.tooltip.button.level", progress)).formatted(Formatting.GRAY);
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		this.textRenderer.draw(matrices, Text.translatable("endallmagic.gui.page.attributes.text.vitality").formatted(Formatting.DARK_GRAY), (this.x + 105) / scaleX.get(), (this.y + 26) / scaleY.get(), 4210752);
		this.textRenderer.draw(matrices, Text.translatable("endallmagic.gui.page.attributes.text.resistances").formatted(Formatting.DARK_GRAY), (this.x + 105) / scaleX.get(), (this.y + 81) / scaleY.get(), 4210752);
	...
	public void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
		...
		this.forEachScreenButton(button -> {
			Identifier key = button.key();
			Identifier lvl = new Identifier("endallmagic_pages:level");
	...
	protected void init() {
		super.init();
		//this.playerData = EndAllMagic.PLAYER_DATA.get(this.client.player);
	...
	static {
		...
		// }, 21, 103));
		COMPONENTS.add(RenderComponent.of(entity -> {
			return Text.translatable("endallmagic.gui.page.attributes.text.movement_speed", ClientUtil.FORMATTING_3.format(entity.getMovementSpeed())).formatted(Formatting.DARK_GRAY);
		}, entity -> {
			List<Text> tooltip = new ArrayList<Text>();
			String formatted = ClientUtil.FORMATTING_3.format(20.0D * entity.getMovementSpeed());
			tooltip.add(Text.translatable("endallmagic.gui.page.attributes.tooltip.movement_speed", formatted).formatted(Formatting.GRAY));
			return tooltip;
		}, 21, 125));
